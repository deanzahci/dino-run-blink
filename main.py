...
def main():
    ...
    for buffer in stream():
        ...
        time_block = np.linspace(current_time, current_time + block_length * dt, block_length, endpoint=False)
        ...
        # AF7 and AF8 RMS
        ax.plot(all_time, all_rms_af7, label='RMS AF7', color='black', linewidth=1, alpha=0.3)
        ax.plot(all_time, all_rms_af8, label='RMS AF8', color='black', linewidth=1, alpha=0.3)
        ...
        for t in blink_times:
            if first_label:
                ax.axvline(x=t, color='red', linestyle='--', linewidth=2, label='Blink Detected')
                first_label = False
            else:
                ax.axvline(x=t, color='red', linestyle='--', linewidth=2)

        # Max RMS
        ax.text(0.05, 0.95, f'Max RMS: {max_value:.2f}', transform=ax.transAxes,
